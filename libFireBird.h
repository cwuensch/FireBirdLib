...
//    };
  ...
  bool        HDD_FindMountPointDevice(const char *AbsPath, char *const OutMountPoint, char *const OutDeviceNode);  // OutMountPoint und OutDeviceNode: max. FBLIB_DIR_SIZE Zeichen (inkl. Nullchar)
  ...
  bool        HDD_GetFileSizeAndInode(const char *Directory, const char *FileName, __ino64_t *CInode, __off64_t *FileSize);
  ...
  bool        HDD_InfBlockSet(const char *AbsRecPath, tinfBlock *infBlock);
...
//    byte                  Zero1;
  ...
  int  OSDMenuGetStringHeight(const char *str, byte fntSize);
  int  OSDMenuGetStringWidth(const char *str, byte fntSize);
  ...
  void OSDKeyboard_Setup(char *Title, char *Variable, dword MaxLength, bool AllowEmpty, bool SaveDict);  // Length zzgl. Null
